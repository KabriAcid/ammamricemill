...
// GET /api/accounts/transactions/v2 - Fetch all transactions with filters
...
// GET /api/accounts/transactions/v2/:id - Get single transaction details
...
// POST /api/accounts/transactions/v2 - Create new transaction
  ...
  try {
    ...
    if (!date || !voucherType || !amount || !fromHeadId || !fromHeadType) {
      return res.status(400).json({
        success: false,
        message: "Date, voucher type, amount, fromHeadId, and fromHeadType are required",
      });
    }
    ...
    const validVoucherTypes = ['receive', 'payment', 'sales_voucher', 'purchase_voucher', 'journal', 'contra'];
    ...
    const [result] = await pool.query(
      ...
      [
        voucherNumber, date, voucherType, partyId || null,
        fromHeadId, fromHeadType, toHeadId || null, toHeadType || null,
        amount, description || null, req.user.id
      ]
    );
...
// PUT /api/accounts/transactions/v2/:id - Update transaction
...
// DELETE /api/accounts/transactions/v2 - Bulk delete transactions
