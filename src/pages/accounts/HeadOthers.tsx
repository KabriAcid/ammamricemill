...
import type { HeadOthers } from "../../types/entities";
...
const HeadOthersPage: React.FC = () => {
  const [data, setData] = useState<HeadOthers[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedRows, setSelectedRows] = useState<number[]>([]);
  const [modalOpen, setModalOpen] = useState<boolean>(false);
  const [editItem, setEditItem] = useState<HeadOthers | null>(null);
  const [filters, setFilters] = useState<Filters>(initialFilters);

  useEffect(() => {
    setLoading(true);
    fetch(`/api/head-others?page=${filters.page}&pageSize=${filters.pageSize}&search=${filters.search}&sort=${filters.sort}&dateFrom=${filters.dateFrom}&dateTo=${filters.dateTo}`)
      .then(res => res.json())
  ...
  const handleCreate = async (item: HeadOthers) => {
    setLoading(true);
    try {
      await fetch("/api/head-others", {
        method: "POST",
  ...
  const handleUpdate = async (item: HeadOthers) => {
    setLoading(true);
    try {
      await fetch(`/api/head-others/${item.id}", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(item),
      });
  ...
  const columns: Array<{ key: string; label: string; render?: (row: HeadOthers) => React.ReactNode }> = [
    { key: "sl", label: "#" },
  ...
  const totalReceive = data.reduce((sum: number, d: HeadOthers) => sum + (d.receive || 0), 0);
  const totalPayment = data.reduce((sum: number, d: HeadOthers) => sum + (d.payment || 0), 0);
  const totalBalance = data.reduce((sum: number, d: HeadOthers) => sum + (d.balance || 0), 0);

  return (
    <div className="space-y-4 animate-fade-in">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <DashboardCard icon="List" title="Total Receive" value={totalReceive.toLocaleString()} />
