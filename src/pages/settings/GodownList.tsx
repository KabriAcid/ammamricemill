...
const GodownList: React.FC = () => {
  ...
  const paginatedGodowns = filteredGodowns.slice(startIndex, startIndex + pageSize);
  ...
  const handleDelete = async (godownIds: string[]) => {
    if (confirm(`Are you sure you want to delete ${godownIds.length} godown(s)?`)) {
      setLoading(true);
      try {
        await new Promise(resolve => setTimeout(resolve, 1000));
        setGodowns(prev => prev.filter(godown => !godownIds.includes(godown.id)));
        setSelectedGodowns([]);
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingGodown) {
        setGodowns(prev => prev.map(godown => 
  ...
  const totalCapacity = godowns.reduce((sum, godown) => sum + godown.capacity, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-primary-600">{godowns.length}</p>
