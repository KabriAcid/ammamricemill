...
export const Stocks: React.FC = () => {
  ...
  const getStockStatus = (quantity: number) => {
    ...
    if (quantity < 1000)
      return { status: "medium", color: "text-blue-600", bg: "bg-blue-100" };
  ...
  const columns = [
    ...
    { key: "lastUpdated", label: "Last Updated", sortable: true },
    {
      ...
      render: (value: number) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-xs font-medium ${bg} ${color}`}
          >
  ...
  return (
    <div className="p-6">
      <Breadcrumb items={[{ label: "Stocks" }, { label: "Main Stocks" }]} />
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <div className="mt-1">
                    {(() => {
                      const { status, color, bg } = getStockStatus(
                        selectedStock.quantity
                      );
                      return (
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-medium ${bg} ${color}`}
                        >
                          {status.charAt(0).toUpperCase() + status.slice(1)}{" "}
                          Stock
                        </span>
