...
import { Plus, CreditCard as Edit, Trash2, Printer, Eye, DollarSign, Users, Calendar } from 'lucide-react';
...
const MonthlySalarySheet: React.FC = () => {
  ...
  const filteredSheets = salarySheets.filter(sheet => {
    const matchesSearch = sheet.description?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleDelete = async (sheetIds: string[]) => {
    if (confirm(`Are you sure you want to delete ${sheetIds.length} salary sheet(s)?`)) {
      setLoading(true);
      try {
        await new Promise(resolve => setTimeout(resolve, 1000));
        setSalarySheets(prev => prev.filter(sheet => !sheetIds.includes(sheet.id)));
        setSelectedSheets([]);
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingSheet) {
        setSalarySheets(prev => prev.map(sheet => 
  ...
  const totalSalaryPaid = filteredSheets.reduce((sum, sheet) => sum + sheet.totalSalary, 0);
  const avgSalaryPerSheet = totalSheets > 0 ? totalSalaryPaid / totalSheets : 0;
  const totalEmployeesPaid = filteredSheets.reduce((sum, sheet) => sum + sheet.totalEmployee, 0);

  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Monthly Salary Sheet</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-secondary-600">₹{Math.round(avgSalaryPerSheet).toLocaleString()}</p>
            <p className="text-sm text-gray-500">Avg Per Sheet</p>
          </div>
        </Card>
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-blue-600">{totalEmployeesPaid}</p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, paymentHead: e.target.value }))}
              className="input-base"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, totalSalary: Number(e.target.value) }))}
                className="input-base"
                min="0"
                required
              />
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">Employee Salary Details</h3>
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-sm text-gray-600">
                Employee salary details table would be implemented here with columns:
                Designation, ID, Name, Salary, Bonus/OT, Absent/Fine, Deduction, Payment, Note, Signature
              </p>
            </div>
          </div>
