...
import { Printer, Calendar, Users, TrendingUp } from 'lucide-react';
...
const MonthlyAttendance: React.FC = () => {
  ...
  const totalMonths = filteredAttendances.length;
  const totalPresent = filteredAttendances.reduce((sum, att) => sum + att.totalPresent, 0);
  const totalAbsent = filteredAttendances.reduce((sum, att) => sum + att.totalAbsent, 0);
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-primary-600">{totalMonths}</p>
            <p className="text-sm text-gray-500">Months Recorded</p>
          </div>
        </Card>
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-green-600">{totalPresent.toLocaleString()}</p>
            <p className="text-sm text-gray-500">Total Present Days</p>
          </div>
        </Card>
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-red-600">{totalAbsent.toLocaleString()}</p>
            <p className="text-sm text-gray-500">Total Absent Days</p>
          </div>
        </Card>
        <Card hover>
          <div className="text-center">
            <p className="text-3xl font-bold text-secondary-600">{Math.round(attendanceRate)}%</p>
      ...
      <Card title="Monthly Attendance Trend" className="mt-6">
        <div className="h-64 bg-gray-50 rounded-lg flex items-center justify-center">
          <div className="text-center">
            <TrendingUp className="w-12 h-12 text-gray-400 mx-auto mb-2" />
            <p className="text-sm text-gray-500">Monthly attendance trend chart would be displayed here</p>
          </div>
        </div>
      </Card>
    </div>
